...
admin_users = ['sudhir', 'aditya.jangid']
dbs = {'command':'command-test.cj8hufhw12ic.us-east-1.rds.amazonaws.com', 'command2':'command-test2.cj8hufhw12ic.us-east-1.rds.amazonaws.com'}
...
def update_permission(info):
    rds_instance,username,permission = info.split()
    rds_host = dbs[rds_instance]
    try:
        conn = pymysql.connect(rds_host, user=admin_name, passwd=admin_password, db=db_name, connect_timeout=10)
    except:
        ...
        sys.exit()
    with conn.cursor() as cur:
        cur.execute( "revoke all privileges on slack.* from '%s'@'%s'" % (username,prcnt))
        cur.execute("grant %s on slack.* to %s@'%s'" %(permission,username,prcnt))
...
def handler(event, context):
    ...
    if command == "reset_password":
        if invoked_by in admin_users:
            reset_password_by_admin(info)
        else:
            reset_password(info)
    if command == "update_permission":
        if invoked_by in admin_users:
            update_permission(info)
        else:
            reset_password(info)
    if command=="create_user" and invoked_by in admin_users:
        create_user(info)
    else:
